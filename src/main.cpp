...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float y = dy + 0.01*sin(g_CameraPhi);                    // Verde
        float z = dz + 0.01*cos(g_CameraPhi)*cos(g_CameraTheta); // Azul
        float x = dx + 0.01*cos(g_CameraPhi)*sin(g_CameraTheta); // Vermelho
